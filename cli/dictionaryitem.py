...
def show(service, version, dictionary_id, item_key):
    ...
    print( dictionaryitem_line.format( 
        **api.dictionaryitem(service, version, dictionary_id, item_key).attrs 
        )
    )
...
def add(service, version, dictionary_id, item_key, item_value):
    ...
    try:
        print( dictionaryitem_line.format(
            **api.dictionaryitempost(service, version, dictionary_id, body)
            )
        )
    except fastly.errors.ConflictError:
